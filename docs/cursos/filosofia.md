---
curso: Filosofia
slug: filosofia
centro: CFH
grau: Licenciatura e Bacharelado
turno: ~
duracao: _A preencher_
coordenacao:
  nome: ~
  email: ~
  telefone: "(48) 3721-4552"
  sala: CFH, 2º andar
  atendimento: "segunda a sexta, 8h às 12h (remoto) e 13h30 às 17h30 (presencial)"
  site: https://filosofia.ufsc.br/
ca:
  nome: Centro Acadêmico Livre de Filosofia
  sigla: CAFIL
  instagram: "@cafil_ufsc"
atletica: ~
instagram_curso: ~
ultima_verificacao: "agosto/2026"
---

# Filosofia

## Sobre o curso

- **Grau / turno:** O curso oferece duas habilitações distintas com turnos diferentes:
  - **Bacharelado** — turno **noturno** (35 vagas)
  - **Licenciatura** — turno **vespertino-noturno** (35 vagas)
- **Duração:** _A preencher_ — verificar nos currículos oficiais em <https://filosofia.ufsc.br/curriculos/> (os PDFs dos currículos 323 – Bacharelado noturno e 307 – Licenciatura não puderam ser acessados diretamente nesta verificação)
- **Página oficial do curso:** <https://filosofia.ufsc.br/>
- **Resumo:** O Bacharelado forma o estudante para a pesquisa em filosofia, culminando em Trabalho de Conclusão de Curso (TCC) sobre tema de livre escolha. A Licenciatura habilita para o magistério de Filosofia no Ensino Médio, exigindo disciplinas pedagógicas e estágio supervisionado. Ambas as modalidades permitem acesso à pós-graduação.

> **Nota:** as informações sobre turnos (noturno para o Bacharelado e vespertino-noturno para a Licenciatura) foram confirmadas via <https://filosofia.ufsc.br/apresentacao/>. A duração em semestres de cada modalidade não pôde ser confirmada com fonte direta acessível nesta verificação — recomenda-se checar os PDFs de currículo em <https://filosofia.ufsc.br/curriculos/> antes de publicar como definitivo.

## Coordenação

- **Coordenador(a):** _A preencher_ — buscas indicam Prof. Ivan Ferreira da Cunha (coordenador) e Prof. Luan Corrêa da Silva (subcoordenador), mas não foi possível confirmar diretamente na página oficial nesta verificação (WebFetch bloqueado); confirmar em <https://filosofia.ufsc.br/expediente/>
- **E-mail:** _A preencher_ — verificar em <https://filosofia.ufsc.br/expediente/> (e-mail exibido como imagem no site, não foi possível capturar nesta verificação)
- **Telefones:** (48) 3721-4552 / (48) 3721-4142
- **Sala / prédio:** CFH, 2º andar, Campus Trindade
- **Atendimento:** segunda a sexta, 8h às 12h (remoto) e 13h30 às 17h30 (presencial)

(ver também [Coordenações](../coordenacoes.md))

## Vida do curso

- **Centro Acadêmico:** Centro Acadêmico Livre de Filosofia (CAFIL) — [@cafil_ufsc](https://www.instagram.com/cafil_ufsc/)
- **Atlética:** _A preencher_ — nenhuma atlética específica de Filosofia verificada; o curso pode participar da **ATCFH**, atlética conjunta do CFH — [@atcfhufsc](https://www.instagram.com/atcfhufsc/) (verificar com o CAFIL)
- **Empresa júnior:** _A preencher_

(ver também [Atléticas e festas](../atleticas-e-festas.md) e [Instagrams](../instagrams.md))

## Dicas de veterano

_A preencher_

## Onde estudar / salas

_A preencher_ (Bloco B do CFH, Campus Trindade — ver [Mapa](../mapa.md))

---

**Fontes consultadas (agosto/2026):**
- <https://filosofia.ufsc.br/apresentacao/> — habilitações (Bacharelado noturno e Licenciatura vespertino-noturno), vagas e estrutura curricular
- <https://filosofia.ufsc.br/expediente/> — telefones da coordenação, horários de atendimento e nomes de coordenador/subcoordenador (Ivan Ferreira da Cunha / Luan Corrêa da Silva) obtidos por indexação de busca; WebFetch direto bloqueado nesta sessão — recomenda-se reconfirmar
- `docs/centros/cfh.md` — CA (CAFIL, @cafil_ufsc) já verificado
- <https://www.instagram.com/atcfhufsc/> — ATCFH (Atlética do CFH), indexação de busca

_Última verificação: agosto/2026 · [Reportar dado desatualizado](https://github.com/maguila-gus25/portal-dos-calouros-ufsc/issues/new?template=atualizacao-conteudo.yml)_
