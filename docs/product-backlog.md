...
| B-74 | 🟠 Should | 🟢 P | ✅ | Como **calouro**, quero **conhecer ferramentas da comunidade** (MyUFSC, MatrUFSC) que facilitam a vida acadêmica, para **usá-las desde a chegada**. (issue #47) **Sprint 17:** seção "Ferramentas da comunidade" em `docs/links-importantes.md` — **MatrUFSC** (`matrufsc.github.io`) e **MyUFSC** (`myufsc.vercel.app`, planeja semestres pelo histórico síntese; link confirmado pelo mantenedor), ambos marcados como não oficiais. |

### E2 — Fichas por curso

| ID | Prioridade | Tam. | Status | História |
|----|-----------|------|--------|----------|
| B-08 | 🟠 Should | 🔴 G | 🚧 | Como **calouro**, quero **uma ficha só do meu curso** (coordenação, CA, atlética, dicas), para **ter tudo num lugar**. **Estado agosto/2026 (pós-Sprint 28):** 112 fichas publicadas (todos os 13 centros). CTC ✅ · CCA ✅ · CSE ✅ · CCS ✅ (coordenadores Sprint 27) · CTJ ✅ (atlética/CA/EJ Sprint 26) · CTS ✅ (coordenadores/atlética/CA/EJ Sprint 26) · CCJ ✅ (coordenação Sprint 27) · CCB ✅ (coordenadora Daniela De Toni + EJ Simbiosis — Sprint 28) · CED ✅ (coordenadores dos 4 cursos + duração Pedagogia — Sprint 28) · CDS ✅ (CAEF Instagram — Sprint 28). CFH: coordenadores sistematicamente `~` (não publicados em fontes verificadas — padrão estabelecido Sprint 21). CFM: alguns campos `_A preencher_` (duração contraditória entre fontes). CCE: maioria completa; alguns campos de atendimento `_A preencher_`. **Sprint 29 (auditoria completa dos ~460 campos `_A preencher_`):** re-verificados todos os campos; preenchidos os localizáveis com fonte (coordenadores CFH — Geografia/História/Antropologia, com correção de nome desatualizado em Geografia; atlética AAAPsiU; e-mail Ciências Sociais; SEMAQUI/CCA; CALICO+CASIN; valor RU; salas Química/Meteorologia/Odontologia). Restante confirmado como estruturalmente bloqueado: coordenadores não publicados (CFH/CFM), e-mails anti-spam por imagem (CTJ/CCJ), atléticas/EJ inexistentes. **Limitação:** WebFetch bloqueado para `*.ufsc.br` na sessão — verificação via WebSearch. Dicas de veterano e "onde estudar" aguardam submissões reais (B-10, B-13 — bloqueados). |
...
| B-47 | 🟠 Should | 🟡 M | ✅ | Como **calouro que busca no Google**, quero **o portal bem indexado** (meta tags dinâmicas, OG image, `sitemap.xml`, `robots.txt`), para **encontrar a informação fácil**. `app/sitemap.ts`, `app/robots.ts` criados; `generateMetadata` com `description` + `openGraph` em todas as páginas. **Fechou sem a OG image**, que constava no escopo — separada em **B-80**. **Pós-#74:** `metadataBase`, `canonical` em todas as rotas, `twitter:card`, JSON-LD (`Organization`, `WebSite`, `FAQPage`, `Course`, `BreadcrumbList`) e fix de conteúdo duplicado (`/secoes/{faq,checklist,mapa}` → 308). |
...
| B-84 | 🟠 Should | 🟢 P | ✅ | Como **calouro**, quero **que a busca também encontre centros de ensino** (ex.: buscar "CCA" ou "Agronomia" retorna a página do centro, não só cursos e seções), para **achar rápido onde estão as coordenações e atléticas de outro centro sem saber de antemão a URL `/centros/<slug>`**. **Grooming Sprint 31 (confirmado lendo o código):** `search()` em `lib/content.ts` hoje varre apenas o `SLUG_MAP` (seções) e `docs/cursos/` — nunca `docs/centros/`, então os 13 centros publicados (B-60) são invisíveis pra busca. `SearchResult["type"]` precisa aceitar `"centro"`; `components/SearchResults.tsx` precisa mapear o novo tipo para `href="/centros/<slug>"` e um badge "Centro". Cobrir com um teste novo em `tests/content.test.ts` (buscar por um centro existente retorna resultado do tipo certo). Sprint-ready: zero dependência de banco/auth/submissão. **Sprint 31:** `SearchResult["type"]` aceita `"centro"`; `search()` varre `docs/centros/` espelhando 1:1 o laço de cursos (mesmo critério de match, mesmo `snippet()`, mesma proteção por `safeMatter()`). `SearchResults.tsx` mapeia o tipo novo para `/centros/<slug>` + badge "Centro"; o ternário por tipo virou dois `switch` sem `default`, então um tipo novo passa a quebrar o build em vez de cair num fallback silencioso. 3 testes unitários novos + 1 e2e. |
...
| B-83 | 🟡 Could | 🟢 P | ✅ | Como **calouro com baixa visão**, quero **que todos os links azuis no corpo das páginas passem em WCAG AA**, para **conseguir lê-los sobre o fundo cinza-claro do app**. (dívida técnica do `ui-ux-review` do Sprint 30) `text-primary` como cor de texto dá **4.33:1** sobre `--background` — passa sobre o card branco (4.75:1), falha no corpo da página. O Sprint 30 criou o token `--primary-link` (`text-primary-link`, 5.45:1 no claro / 5.25:1 no escuro) e o aplicou no `SugerirCorrecao`, mas os links pré-existentes que ainda usam `text-primary` sobre `--background` — ex.: "Voltar para o início" em `/secoes/[slug]`, `/cursos/[slug]`, `/centros/[slug]`, `/faq`, `/checklist`, `/mapa` — seguem em 4.33:1. Varrer e migrar. Ratios em `docs/identidade-visual.md`. **Sprint 31:** varredura concluída — links "Voltar…" das seis rotas, `Footer`, estado ativo do `NavLinks`, seletores `[&_a]` das seções vindas do Markdown e as regras `a`/`.prose-content a` do `globals.css` migrados para `text-primary-link`. A varredura revelou uma falha **pior que a original**: nos chips/badges de link (`LinkCardGrid`, `InstagramSection`) o texto fica sobre superfície *tingida* (`bg-primary/10`, `/20` no hover) — 4.14:1 e 3.60:1 — e no escuro `--primary-link` tem a mesma lightness de `--primary`, então não resolve (a tinta *clareia* o fundo; o texto precisa ir na direção oposta à do claro). Daí o quarto token **`--primary-on-tint`** (claro 45%, escuro 70%): chip 5.22:1 / hover 4.53:1 no claro, 5.51:1 / 4.75:1 no escuro. Ícones `aria-hidden` e `bg-primary*` deliberadamente **não** migrados (AA de texto não se aplica a decoração) — decisão documentada com ratios em `docs/identidade-visual.md`. |
| B-86 | 🟠 Should | 🟡 M | ⬜ | Como **mantenedor**, quero **testes automatizados que chamem as rotas de API (`app/api/**/route.ts`) diretamente**, para **detectar quando um Route Handler quebra o contrato JSON/status HTTP mesmo que o loader (`lib/content.ts`) continue correto**. **Grooming Sprint 31 (confirmado lendo o código):** o Vitest do B-81 testa `getSection`/`getCourse`/`getCenter`/`search` diretamente, mas nenhum teste chama o `GET()` exportado pelos 8 arquivos de `app/api/`; o e2e (B-54) só bate em `/api/health`. Uma regressão só no wrapper (`NextResponse.json`, status 404 quando o slug não existe, parsing de `searchParams`) hoje não quebra `npm test`. Cobrir em `tests/` (novo arquivo, ex. `tests/api.test.ts`, `environment: node`, chamando `GET(new Request(url), { params: Promise.resolve({ slug }) })` conforme a assinatura Next 15): `/api/health` (200), `/api/sections` e `/api/sections/[slug]` (200 válido + 404 inválido), `/api/courses` e `/api/courses/[slug]` (idem), `/api/centros` e `/api/centros/[slug]` (idem), `/api/search?q=` (resultado não vazio, vazio, sem `q`). Sprint-ready: só adiciona teste, não muda comportamento. |
| B-87 | 🟠 Should | 🟢 P | ✅ | Como **mantenedor**, quero **um script que valide o frontmatter de `docs/cursos/**` e `docs/centros/**` e rode no CI a cada PR**, para **um PR de conteúdo com YAML malformado ou campo obrigatório ausente falhar de forma visível, em vez da ficha sumir silenciosamente do site em produção**. **Grooming Sprint 31:** `safeMatter()` (adicionado no Sprint 30 por causa de um bug real — frontmatter malformado em um único arquivo derrubava `/api/courses`, `/api/centros` e o build inteiro) degrada graciosamente por desenho, e isso é o comportamento certo em produção — mas tem um efeito colateral: hoje um PR de conteúdo pode mergear um YAML quebrado sem o CI reclamar, porque `listCourses()`/`listCenters()` simplesmente pulam o arquivo e a ficha só desaparece, sem erro nenhum em lugar nenhum. Novo script `scripts/validate-frontmatter.mjs`: roda `gray-matter` sobre todo `docs/cursos/*.md` e `docs/centros/*.md`, falha (`exit 1`) se o parse lançar exceção, e verifica campos mínimos obrigatórios (`slug`, `curso`/`titulo`, e `centro` quando aplicável nas fichas de curso). Novo job (`npm run validate:content` ou similar) em `.github/workflows/unit.yml` ou workflow próprio, rodando em PRs para `main`. **Não** reescrever `safeMatter()` nem seu comportamento de produção — este item é só a rede de segurança no PR, antes do merge. Sprint-ready: script + workflow novos, sem tocar em código de runtime. **Sprint 31:** `scripts/validate-frontmatter.mjs` varre os 84 arquivos (71 cursos + 13 centros); falha com o caminho e uma explicação em português para quem não sabe o que é YAML. `_A preencher_` segue sendo valor **válido** — a convenção do projeto para dado não confirmado não pode virar pressão para inventar dado. Roda como step do job `unit` existente (mesmo gatilho, ~200ms) para não somar um required check novo à branch protection. Validado quebrando um arquivo de propósito nos dois modos de falha. `safeMatter()` e o runtime intocados. |
| B-88 | 🟡 Could | 🟢 P | ✅ | Como **mantenedor**, quero **testes Playwright de fumaça para `/centros` e `/centros/[slug]`**, para **detectar regressão na navegação por centro (E14) antes do deploy**. **Grooming Sprint 31 (confirmado lendo o código):** `e2e/smoke.spec.ts` tem 8 testes (home, skip link, `/faq`, `/checklist`, `/mapa`, `/api/health`, busca, `/cursos/[slug]`) e nenhum visita `/centros` ou `/centros/[slug]` — apesar de ser o fluxo principal de descoberta de conteúdo desde o Sprint 16 (E14) e cobrir 13 centros e 112 fichas. Adicionar 2 casos: `/centros` carrega e lista pelo menos um card de centro; `/centros/ctc` carrega e contém pelo menos um link para `/cursos/<slug>` (o mesmo critério já previsto nos critérios de aceite do B-66, nunca virou teste). Sprint-ready: extensão de um spec já existente. **Sprint 31:** 4 casos novos em `e2e/smoke.spec.ts` (`/centros` lista centros; `/centros/ctc` lista cursos; ficha de curso linka de volta ao centro; busca encontra centro), sem fixar contagem. **Achado:** a suíte rodava contra `npm run dev`, onde cada rota compila na primeira visita — com workers em paralelo, 2 ou 3 testes *diferentes* falhavam a cada rodada, mascarado no CI por `workers: 1` + `retries: 2`. Passa a servir o build de produção, como o Lighthouse CI já faz. **12/12 estável.** |
...
| B-60 | 🟠 Should | 🔴 G | ✅ | Como **calouro de qualquer centro da UFSC Florianópolis**, quero **conteúdo sobre meu centro** (coordenações, atléticas, links úteis), para **me virar no primeiro dia mesmo não sendo do CTC**. Cobre: CCE, CCS, CCJ, CFH, CFM, CCB, CSE, CCA (Itacorubi), CED, CDS (Centro de Desportos e Saúde), CA (Araranguá), CA (Joinville). **Sprint 13:** infra + CCA + CSE. **Sprint 14:** CCE e CCS. **Sprint 18:** CCJ (Direito), CFH (9 cursos) e CFM (5 cursos) publicados — 8 centros. **Sprint 22:** CCB publicado — 9 centros. **Sprint 23:** CED + CDS — 11 centros. **Sprint 24:** CTJ (Joinville, 8 cursos) e CTS (Araranguá, 5 cursos) — **13 centros. B-60 ✅ fechado.** |
| B-61 | 🟠 Should | 🔴 G | ✅ | Como **calouro**, quero **fichas dos cursos de todos os centros da UFSC Florianópolis**, para **ter informações do meu curso independente do centro**. Seguir o mesmo modelo de `docs/cursos/<slug>.md` já usado no CTC. **Sprint 14:** 4 fichas do CCA. **Sprint 16:** 5 do CSE. **Sprint 18:** 6 do CCS. **Sprint 19:** 9 do CCE — 5 centros originais com fichas completas. **Sprint 20:** 5 fichas do CFM + Direito (CCJ). **Sprint 21:** 9 fichas do CFH (Psicologia, Filosofia, História, Antropologia, Ciências Sociais, Museologia, Geografia, Geologia, Licenciatura Intercultural Indígena) — **todos os 8 centros publicados agora têm fichas completas**. Total: 50 fichas. Nota: novos centros (CCB, CED, CDS…) precisarão de fichas próprias quando publicados — B-60. |
...
| B-76 | 🔴 Must (fix) | 🟢 P | ✅ | Como **calouro**, quero **que o marcador do CFM no mapa esteja no lugar certo**, para **me localizar corretamente**. (issue #39) **Sprint 17:** coordenadas do CFM em `components/MapView.tsx` corrigidas para `-27.5994566, -48.5233276` (fornecidas pelo mantenedor). |
...
| B-68 | 🟠 Should | 🟢 P | ✅ | Como **calouro de outro centro**, quero **que `/cursos` me redirecione para `/centros`** em vez de listar apenas os cursos do CTC, para **não ser enganado achando que o portal só tem cursos do CTC**. **Superado pelo B-75 (Sprint 17):** `/cursos` voltou a ser uma listagem, agora **global por centro** (não mais só CTC), resolvendo o problema original sem redirect. |
...
| B-75 | 🟠 Should | 🟡 M | ✅ | Como **calouro que já sabe o curso**, quero **uma aba "Cursos" na topbar e uma página `/cursos` com todos os cursos** (agrupados por centro), para **chegar à ficha sem passar por Centros → centro → cursos**. (issue #45) **Sprint 17:** `/cursos` deixou de redirecionar e virou listagem global por centro; aba "Cursos" no `NavLinks`; `/cursos` de volta no sitemap. |
| B-77 | 🔴 Must (fix) | 🟢 P | ✅ | Como **usuário**, quero **que o nome do site não seja mais "UFSC — CTC"** nos metadados, para **refletir que o portal cobre toda a UFSC Florianópolis**. (issue #40) **Sprint 17:** `app/layout.tsx` (title/description/OG) e `app/manifest.ts` (PWA) sem "CTC". |
| B-85 | 🟠 Should | 🟢 P | ✅ | Como **calouro lendo a ficha do meu curso**, quero **que o nome do centro no topo da ficha seja um link para a página do centro**, para **chegar às coordenações, atléticas e outros cursos do meu centro sem digitar a URL manualmente**. **Grooming Sprint 31 (confirmado lendo o código):** em `app/cursos/[slug]/page.tsx`, o rótulo `<p>{course.centro}</p>` (ex.: "CTC") é texto puro, não link — o calouro chega à ficha do curso pela busca ou pelo `/centros/[slug]` mas não consegue voltar para o centro a partir do curso. `course.centro` já é a sigla maiúscula que bate com o `slug` do centro em `docs/centros/*.md` (confirmado: `centro: CTC` no frontmatter do curso ↔ `slug: ctc` no centro), então basta `course.centro.toLowerCase()` — sem mapa novo nem alteração de `lib/content.ts`. Tratar o caso `course.centro` nulo (não renderizar link). Considerar também ajustar a `breadcrumbSchema` de `Início → Cursos → {Curso}` para `Início → {Centro} → {Curso}` se ficar coerente com o fluxo entregue em B-66/B-71. Sprint-ready: puramente frontend, um arquivo. **Sprint 31:** o slug do centro é resolvido por `getCenter()`, não só por `toLowerCase()` — sigla sem centro correspondente cai no `<p>` de texto puro, sem link morto; `centro` ausente não renderiza nada. O `breadcrumbSchema` passa a `Início → Centros → {Centro} → {Curso}` quando o centro é resolvido, usando o mesmo `center.title` já publicado em `/centros/[slug]` (não é dado inventado); sem centro resolvido, mantém os 3 níveis. Alvo de toque de 44px via padding compensado por margem negativa. |
...
> submissões reais de veterano) — isso não muda sem decisão do mantenedor sobre v2.0.
> **Grooming pós-Sprint 30:** o backlog executável deixou de estar vazio. Cinco itens
> novos (B-84 a B-88) nasceram de ler o código com atenção — nenhum inventado, todos
> apontam para uma lacuna real já confirmada no arquivo citado na história. Todos são
> 100% executáveis agora: sem banco, sem auth, sem depender de submissão de veterano.
> Quem decide a composição final do sprint é o Scrum Master via `/sprint-plan`.
...
2. **B-84** — busca passa a indexar os 13 centros, não só seções e cursos
   (frontend-dev/`lib/content.ts`) — Should, P.
3. **B-85** — nome do centro na ficha do curso vira link para `/centros/[slug]`
   (frontend-dev) — Should, P.
4. **B-86** — testes automatizados das 8 rotas de `app/api/` (tester) — Should, M.
5. **B-87** — script de validação de frontmatter de `docs/cursos`/`docs/centros`
   no CI (tester/debugger) — Should, P. Fecha a lacuna que o bug do Sprint 30
   deixou: hoje um PR de conteúdo quebrado mergeia sem o CI reclamar.
6. **B-88** — Playwright cobre `/centros` e `/centros/[slug]` (tester) — Could, P.
7. **B-37 + B-50 + E13** — formulário de histórias + banco + auth OAuth. **Requer
...
> **Sugestão de composição (não vinculante — decisão é do Scrum Master):** B-84 +
> B-85 + B-87 + B-83 formam um sprint pequeno e coeso (todos P, frontend/conteúdo,
> baixo risco, alto valor imediato pro calouro e pro mantenedor). B-86 e B-88 são
> puramente de teste/CI e podem entrar juntos numa rodada de qualidade, ou serem
> distribuídos entre sprints de conteúdo para não concentrar todo o sprint em
> dívida técnica.
