...
| B-73 | 🟠 Should | 🔴 G | 🚧 | Como **calouro**, quero **que as páginas de seção tenham UI adequada ao tipo de dado** (não markdown renderizado cru), para **consumir a informação mais rápido no celular**. (issue #48) **Sprint 17 (1º passo):** `lib/content.ts` expõe `Section.blocks` (parse via `marked.lexer`, `docs/` como fonte única); 3 seções com componente dedicado — links, datas, ru — com fallback `.prose-content`. **Sprint 18:** + instagrams (grid de perfis com badge oficial/estudantil) e historias (relatos em cards). Faltam: faq/checklist/mapa (têm páginas próprias, baixa prioridade). |
...
| B-60 | 🟠 Should | 🔴 G | 🚧 | Como **calouro de qualquer centro da UFSC Florianópolis**, quero **conteúdo sobre meu centro** (coordenações, atléticas, links úteis), para **me virar no primeiro dia mesmo não sendo do CTC**. Cobre: CCE, CCS, CCJ, CFH, CFM, CCB, CSE, CCA (Itacorubi), CED, CDS (Centro de Desportos e Saúde), CA (Araranguá), CA (Joinville). **Sprint 13:** infra + CCA + CSE. **Sprint 14:** CCE e CCS. **Sprint 18:** CCJ (Direito), CFH (9 cursos) e CFM (5 cursos) publicados — 8 centros no total. Faltam: CCB, CED, CDS e campi de Joinville e Araranguá. |
| B-61 | 🟠 Should | 🔴 G | 🚧 | Como **calouro**, quero **fichas dos cursos de todos os centros da UFSC Florianópolis**, para **ter informações do meu curso independente do centro**. Seguir o mesmo modelo de `docs/cursos/<slug>.md` já usado no CTC. **Sprint 14:** 4 fichas do CCA. **Sprint 16:** 5 do CSE. **Sprint 18:** 6 do CCS. **Sprint 19:** 9 do CCE (Animação, Artes Cênicas, Cinema, Design, Design de Produto, Jornalismo, Letras Estrangeiras, Letras Português, Secretariado) — reaproveitando `docs/centros/cce.md`; incertos como `_A preencher_`. **Marco:** 5 centros originais (CTC/CCA/CSE/CCS/CCE) com fichas completas. **Sprint 20:** 5 fichas do CFM (Física, Matemática, Química, Meteorologia, Oceanografia) + Direito (CCJ) — reaproveitando `cfm.md`/`ccj.md`. Falta: cursos do CFH (9, dados escassos — sprint próprio). |
