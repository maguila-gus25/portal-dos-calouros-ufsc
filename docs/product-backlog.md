...
| B-84 | 🟠 Should | 🟢 P | ⬜ | Como **calouro**, quero **que a busca também encontre centros de ensino** (ex.: buscar "CCA" ou "Agronomia" retorna a página do centro, não só cursos e seções), para **achar rápido onde estão as coordenações e atléticas de outro centro sem saber de antemão a URL `/centros/<slug>`**. **Grooming Sprint 31 (confirmado lendo o código):** `search()` em `lib/content.ts` hoje varre apenas o `SLUG_MAP` (seções) e `docs/cursos/` — nunca `docs/centros/`, então os 13 centros publicados (B-60) são invisíveis pra busca. `SearchResult["type"]` precisa aceitar `"centro"`; `components/SearchResults.tsx` precisa mapear o novo tipo para `href="/centros/<slug>"` e um badge "Centro". Cobrir com um teste novo em `tests/content.test.ts` (buscar por um centro existente retorna resultado do tipo certo). Sprint-ready: zero dependência de banco/auth/submissão. |
